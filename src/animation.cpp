...
Animation::Animation(const Config &config, const Terminal &terminal)
...
void Animation::play(const Text &_text, std::function<void()> _on_stop) {
  text = &_text;
  on_stop = _on_stop;
  init();
  is_playing = true;
  timer_watcher.set<Animation, &Animation::tick>(this);
...
void Animation::tick(ev::timer &w, int revents) {
  ...
  for (size_t col = 0; col < terminal.getWidth(); ++col) {
    ...
    int col_end = tick_id - col_offsets[col] - col_lengths[col];
...
wchar_t Animation::getRandSymbol() {
